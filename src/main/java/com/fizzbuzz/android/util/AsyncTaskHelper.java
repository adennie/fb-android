 ...
 * <li>implements a interface (AsyncTaskControllee) to assist client code in overseeing multiple asynchronous tasks
 * <li>delivers exceptions emanating from the background task to the client's foreground thread
 * <li>allows an arbitrary number of parameters of arbitrary types to be passed to the background job (unlike
 * AsyncTask.doInBackground(Params...))
 * </ol>
 * <p>
 * Unlike AsyncTask, where you implement a subclass to define task-specific behavior, with AsyncTaskHelper you supply
 * that behavior to its constructor via an {@link AsyncTaskJob} and (optionally) an {@link AsyncTaskResultProcessor}. By
 * instantiating AsyncTaskJobs on the fly in the context of the method initiating the task, the code inside the job has
 * access to the local and class member variables available to the context of that calling method, obviating the need to
 * package that contextual information into something that works with the "Params..." signature of doInBackground().
 * <p>
 * AsyncTaskHelper provides plumbing to receive progress updates from the AsyncTaskJob running on the background thread,
 * and to forward those updates to the {@link ProgressListener} interface of an object the UI thread. In addition to a
 * numeric indicator of progress completion, this plumbing also allows the background job to provide two messages that
 * describe the ongoing work. This information maps well to a ProgressDialog with numeric progress and up to two status
 * messages (e.g. showing that a task that is 40% complete, currently executing "step 2", "subtask c").
 * <p>
 * It also provides facilities for client code to reconnect to a task, interrogating it to determine whether it is still
 * running, what it is currently doing, and how much progress it has made. This can be useful when the original calling
 * context is replaced by a new context that wants to pick up where the prior one left off (e.g. when an Activity that
 * has been recreated due to device rotation wants to display ongoing progress to the user, post-rotation).
 * <p>
 * AsyncTaskHelper assists with exception handling by putting a try/catch block around the execution of the background
 * job, saving any thrown exceptions and passing them to the provided AsyncTaskResultProcessor on the foreground thread,
 * to be handled as needed.
 * <p>
 * An AsyncTaskHelper may optionally be registered with and controlled by an {@link AsyncTaskController} via its
 * {@link AsyncTaskControllee} interface. This allows higher-level code which may be interacting with multiple
 * AsyncTaskHelpers to keep track of them and manage them more easily. </ul>
 * 
 * @author Andy Dennie
 * 
 * @param <Result>
